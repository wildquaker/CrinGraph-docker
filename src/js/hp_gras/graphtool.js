      ...
      <div class="graphBox" data-sticky-graph="`+ alt_sticky_graph +`" data-animated="`+ alt_animated +`">
        ...
        <div class="customDF">
          <span>Preference Adjustments:</span>
            ...
            <button class="spinner increment tilt">&#9650;</button>
            <button class="spinner decrement tilt">&#9660;</button>
            <div>
                <input type="number" inputmode="decimal" id="cusdf-bass" value="`+ default_bass_shelf +`" step="1""></input>
                <span>Bass (dB)</span>
            </div>
            <button class="spinner increment bass">&#9650;</button>
            <button class="spinner decrement bass">&#9660;</button>
            <div>
                <input type="number" inputmode="decimal" id="cusdf-treb" value="`+ default_treble +`" step="0.1""></input>
                <span>Treble (dB)</span>
            </div>
            <button class="spinner increment treble">&#9650;</button>
            <button class="spinner decrement treble">&#9660;</button>
          <div>
            <input type="number" inputmode="decimal" id="cusdf-ear" value="`+ default_ear +`" step="0.1""></input>
            <span>Ear Gain (dB)</span>
          </div>
          <button class="spinner increment ear">&#9650;</button>
          <button class="spinner decrement ear">&#9660;</button>
...
let tickPattern = [3,0,0,1,0,0,2,0],
    ...
    tickThickness = [2,4,4,7,12].map(t=>t/10);
...
function getCurveColor(id, o, hex) {
    ...
    let s = Math.sin(2*Math.PI*i);
        return d3.hcl(360*((i + t/p2)%1), // hue varies with "o"
...
function isActive(p) {
    return p.isActive || activePhones.find(q=>q.fileName===p.fileName);
}
function setPhoneTr(phtr) {
    phtr.each(function (p) {
        p.highlight = isActive(p);
    ...
    phtr.style("background",p=>p.isTarget&&!isActive(p)?null:getDivColor(p.id,p.highlight,p.hexColor))
...
function setBaseline(b, no_transition) { // Edited here - Gat
    baseline = b;
    updateYCenter();
    let duration = 500
    if (no_transition) duration = 0;
    //clearLabels();
...
function getAdjustedTarget(activePhones) {
    let target = activePhones.find(p => p.isTarget && p.isAdjusted);
    return target;
}
function isAdjusted(p) {
    return p.isAdjusted;
}
...
function addPhonesToUrl() {
    ...
    let title = baseTitle,
        ...
        adjTarget = getAdjustedTarget(activePhones);
...
const increment = (type) => {
    ...
    $('input#'+type).val(( Number(input.val()) + step).toFixed(1));
}
...
$('.spinner.increment.bass').click(() => {
    increment("cusdf-bass");
    boost = Number($('input#cusdf-bass').val());
    updateDF(boost, tilt, ear, treble, "bass");
});
$('.spinner.decrement.bass').click(() => {
    decrement("cusdf-bass");
    boost = Number($('input#cusdf-bass').val());
    updateDF(boost, tilt, ear, treble, "bass");
});
$('.spinner.increment.tilt').click(() => {
    increment("cusdf-tilt");
    tilt = Number($('input#cusdf-tilt').val());
    updateDF(boost, tilt, ear, treble, "tilt");
});
$('.spinner.decrement.tilt').click(() => {
    decrement("cusdf-tilt");
    tilt = Number($('input#cusdf-tilt').val());
    updateDF(boost, tilt, ear, treble, "tilt");
});
$('.spinner.increment.treble').click(() => {
    increment("cusdf-treb");
    treble = Number($('input#cusdf-treb').val());
    updateDF(boost, tilt, ear, treble, "treble");
});
$('.spinner.decrement.treble').click(() => {
    decrement("cusdf-treb");
    treble = Number($('input#cusdf-treb').val());
    updateDF(boost, tilt, ear, treble, "treble");
});
$('.spinner.increment.ear').click(() => {
    increment("cusdf-ear");
    ear = Number($('input#cusdf-ear').val());
    updateDF(boost, tilt, ear, treble, "ear");
});
$('.spinner.decrement.ear').click(() => {
    decrement("cusdf-ear");
    ear = Number($('input#cusdf-ear').val());
    updateDF(boost, tilt, ear, treble, "ear");
});
...
function showPhone(p, exclusive, suppressVariant, trigger) {
    ...
    } else if (p.isTarget && !tiltableTargets.includes(p.dispName) && !isAdjusted(p)) {
        customTiltName = p.dispName;
        if (p.dispName == "∆") customTiltName = "Delta (∆)";
...
// Edits for php token function start (DO NOT DELETE DURING UPDATES)
function parse_query_string(query) { // Gat
  var vars = query.split("&");
  var query_string = {};
  for (var i = 0; i < vars.length; i++) {
    var pair = vars[i].split("=");
    var key = decodeURIComponent(pair[0]).replace(/_/g," ");
    var value = decodeURIComponent(pair[1]).replace(/_/g," ");
    // If first entry with this name
    if (typeof query_string[key] === "undefined") {
      query_string[key] = decodeURIComponent(value).replace(/_/g," ");
      // If second entry with this name
    } else if (typeof query_string[key] === "string") {
      var arr = [query_string[key].replace(/_/g," "), decodeURIComponent(value).replace(/_/g," ")];
      query_string[key] = arr;
      // If third or later entry with this name
    } else {
      query_string[key].push(decodeURIComponent(value).replace(/_/g," "));
    }
  }
  return query_string;
}
...
d3.json(typeof PHONE_BOOK !== "undefined" ? PHONE_BOOK
    ...
    if (ifURL) {
        ...
        _isAdjusted = url.includes(cDFb) || url.includes(cDFt) || url.includes(cDFtr) || url.includes(cDFe);
    }
    ...
    if (targets) {
        let b = window.brandTarget = { name:"Targets", active:false },
            ...
            ph = t => ({
                ...
                dispName:customTargetDispNames && customTargetDispNames[t] ? customTargetDispNames[t] : t, phone:t, fullName:(customTargetDispNames && customTargetDispNames[t] ? customTargetDispNames[t] : t)+" Target", fileName:t+" Target"
            });
    ...
    // df = inits.find(p => p.isTarget);
    // console.log(df);
    // if(df) {
    //     if(_isAdjusted) {
    //     showPhone(df);
    //     updateDispVals();
    //     inits = inits.filter(p => p !== df);
    //     }
    ...
    updateDF = (boost, tilt, ear, treble, change) => {
        ...
        if (activeTarget.isTarget && !tiltableTargets.includes(activeTarget.dispName) && !isAdjusted(activeTarget)) {
        ...
        let customDispName = customTiltName;
        if(customTargetDispNames) {
            let key = Object.keys(customTargetDispNames).find(key => customTargetDispNames[key] === customTiltName);
            if(key) {
                customDispName = key;
            }
        }

        let phoneObj = { isTarget:true, brand:brand, phone:customTiltName,
            fullName:customTiltName + preferenceAdjustments,
            dispName:customTiltName + preferenceAdjustments,
            fileName:customDispName + " Target",
            isAdjusted:true
        };
        ...
        let oldPhoneObj = phoneObjs.find(p => p.isAdjusted);
        if (oldPhoneObj) {
...
function addExtra() {
    ...
    document.querySelector("div.extra-eq button.export-filters").addEventListener("click", () => {
        ...
        if(!phoneObj.fullName)
            phoneObj.fullName = phoneObj.dispName;
        filters.forEach((f, i) => {
